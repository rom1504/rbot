...
let bot,inventory,nearest,isEmpty,isNotEmpty,blocks,async;

function init(_bot,_inventory,_nearest,_isEmpty,_isNotEmpty,_blocks,_async)
{
	bot=_bot;
...
{
	const a=[];
	for(let i in bot.entities) a.push(bot.entities[i]);
	return a;
}
...
function stringToAbsolutePosition(s,u,head,done)
{
	...
	stringToBlock(s,function(b){
		...
		{
			let e;if((e=stringToEntity(s,u))!=null) {done(head!=null ? e.position.offset(0,e.height,0) : e.position);return}
